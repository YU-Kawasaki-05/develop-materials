...
```mermaid
graph LR
    subgraph "GitHub Flow 2024"
        Main[main<br/>常時デプロイ可能] 
        Feature1[feature/user-auth<br/>機能開発]
        Feature2[feature/api-v2<br/>API更新]
        Feature3[hotfix/security-patch<br/>緊急修正]
        
        Main --> Feature1
        Main --> Feature2
        Main --> Feature3
        
        Feature1 --> |PR + Review + CI| Main
        Feature2 --> |PR + Review + CI| Main
        Feature3 --> |PR + Review + CI| Main
        
        Main --> Deploy[自動デプロイ<br/>本番環境]
    end
    
    style Main fill:#4CAF50,color:white
    style Deploy fill:#FF5722,color:white
```
...
```mermaid
graph TB
    subgraph "Git-flow Enterprise"
        Master[master/main<br/>本番リリース版]
        Develop[develop<br/>次期バージョン統合]
        Feature[feature/*<br/>機能開発]
        Release[release/*<br/>リリース準備]
        Hotfix[hotfix/*<br/>緊急修正]
        Support[support/*<br/>長期サポート]
    ...
    style Master fill:#4CAF50,color:white
    style Develop fill:#2196F3,color:white
    style Release fill:#FFC107,color:black
    style Hotfix fill:#F44336,color:white
    style Support fill:#9C27B0,color:white
```
...
```mermaid
graph TB
    subgraph "GitLab Flow with Environment Branches"
        Main[main<br/>統合ブランチ]
        Feature[feature/*<br/>機能開発]
        
        subgraph "Environment Branches"
            Staging[staging<br/>ステージング環境]
            PreProd[pre-production<br/>本番前環境]
        ...
        subgraph "Release Branches"
            Release1[release/v1.x<br/>バージョン1系]
            Release2[release/v2.x<br/>バージョン2系]
        end
    ...
    style Main fill:#FC6D26,color:white
    style Production fill:#4CAF50,color:white
    style Staging fill:#FF9800,color:white
```
...
```mermaid
graph TD
    subgraph "Multi-repo Architecture"
        subgraph "Frontend Repositories"
            WebRepo[web-frontend-repo]
            MobileRepo[mobile-app-repo] 
            AdminRepo[admin-dashboard-repo]
        end
        
        subgraph "Backend Repositories"
            UserRepo[user-service-repo]
            PaymentRepo[payment-service-repo]
            NotificationRepo[notification-service-repo]
        end
        
        subgraph "Shared Libraries"
            UIRepo[ui-components-repo]
            UtilsRepo[utilities-repo]
        ...
        UserRepo --> TypesRepo
        PaymentRepo --> TypesRepo
        NotificationRepo --> TypesRepo
    end
    
    style WebRepo fill:#e3f2fd
    style UserRepo fill:#e8f5e8
    style UIRepo fill:#fff3e0
```
